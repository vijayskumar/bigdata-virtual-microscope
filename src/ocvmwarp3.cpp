...
int main(int argc, char *argv[])
{
    ...
    int8 cachesize = MB_128;
    bool rowwise = false;
    bool chunkwise = false;
    int nchunkwise = -1;
    int readahead = 10;
    ...
    while (argc > 1) {
        ...
        else if (!strcmp(argv[1], "--rowwise")) {
            rowwise = true;
        }
        else if (!strcmp(argv[1], "--chunkwise")) {
            chunkwise = true;
        }
        else if (!strcmp(argv[1], "--readahead")) {
            readahead = atoi(argv[2]);
            dcmpi_args_shift(argc, argv);
        }
        else if (!strcmp(argv[1], "--nchunkwise")) {
            nchunkwise = atoi(argv[2]);
            assert(nchunkwise >= 1);
            dcmpi_args_shift(argc, argv);
        }
        else if (!strcmp(argv[1], "--cachesize")) {
            cachesize = dcmpi_csnum(argv[2]);
            dcmpi_args_shift(argc, argv);
        }
        else {
            break;
    ...
    if (rowwise && chunkwise ||
        rowwise && nchunkwise!=-1 ||
        chunkwise && nchunkwise!=-1) {
        std::cerr << "ERROR: only set one of rowwise, chunkwise, nchunkwise"
    ...
    std::vector<DCFilterInstance*> warpers;
    for (u2 = 0; u2 < warp_filters_per_host; u2++) {
        std::vector<DCFilterInstance*> warpers_this_thread_id;
        std::vector<DCFilterInstance*> rangefetchers;
        for (u = 0; u < hosts.size(); u++) {
            std::string uniqueName = "W_" + tostr(u) + "_" + tostr(u2);
            DCFilterInstance * warper =
                new DCFilterInstance("ocvm_warper", uniqueName);
            layout.add(warper);
            warpers.push_back(warper);
            warpers_this_thread_id.push_back(warper);
            warper->bind_to_host(hosts[u]);
            if (rowwise) {
                warper->set_param("algo", "rowwise");
            }
            else if (chunkwise) {
                warper->set_param("algo", "chunkwise");
            }
            else if (nchunkwise!=-1) {
                warper->set_param("algo", "nchunkwise");
                warper->set_param("nchunks_at_a_time", tostr(nchunkwise));
            }
            else {
                warper->set_param("algo", "simple");
            }
            warper->set_param("cachesize", tostr(cachesize));
            ...
            layout.add_port(warper, "to_console", &console, "from_warper");

            DCFilterInstance * rangefetcher = new DCFilterInstance(
                "ocvm_mediator_rangefetcher",
                tostr("rf") + tostr(u) + "_" + tostr(u2));
            rangefetcher->bind_to_host(hosts[u]);
            rangefetcher->set_param("readahead", tostr(readahead));
            layout.add(rangefetcher);
            layout.add_port(rangefetcher, "0",
                            warper, "from_rangefetcher");
            layout.add_port(warper, "to_rangefetcher",
                            rangefetcher, "0");
            rangefetchers.push_back(rangefetcher);
        }
        mediator_add_client(layout, info, warpers_this_thread_id);
...
*/
    ...
    for (u = 0; u < descriptor.parts.size(); u++) {
        DCBuffer * in = console_filter->read("from_warper");
        ImageCoordinate ic;
